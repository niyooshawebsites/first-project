...
class StudentController extends Controller
{
    ...
    // // whereAny clause
    // public function whereClause5()
    // {
    //     // $result = Student::where('age', 25)->orWhere('score', 25)->count();

    //     // either age or score should be 25
    //     // $result = Student::whereAny(['age', 'score'],  25)->count();

    //     // both age and score should be 25
    //     $result = Student::whereAll(['age', 'score'], 25)->count();
    //     return response()->json($result, 200)->withHeaders([
    //         'Content-Type' => 'application/json',
    //     ]);
    // }

    // // get specific students based on age, score using scope method - reusable model method 
    // public function getSpecificMale()
    // {
    //     $result = Student::getSpecificStudents(25, 25, 'm');
    //     return response()->json($result, 200)->withHeaders([
    //         'Content-Type' => 'application/json',
    //     ]);
    // }
    ...
    // // get specific students based on age, score using scope method - reusable model method 
    // public function getSpecificFemale()
    // {
    //     $result = Student::getSpecificStudents(20, 20, 'f');
    //     return response()->json($result, 200)->withHeaders([
    //         'Content-Type' => 'application/json',
    //     ]);
    // }
    ...
    // {
    //     $result = Student::onlyTrashed()->get();
    //     return response()->json($result, 200)->withHeaders([
    //         'Content-Type' => 'application/json',
    //     ]);
    // }
    ...
    // public function restoreTrashedStudents($id)
    // {
    ...
    // {
    //     $result = Student::findorFail($id)->forceDelete();
    //     return response()->json($result, 200)->withHeaders([
    //         'Content-Type' => 'application/json',
    //     ]);
    // }

    // // force delete trashed students
    // public function forceDeleteTrashedStudents($id)
    // {
    //     $result = Student::onlyTrashed()->findorFail($id)->forceDelete();
    //     return response()->json($result, 200)->withHeaders([
    //         'Content-Type' => 'application/json',
    //     ]);
    // }
    ...
    public function readStudents(Request $request)
    {
        $result = Student::when($request->search, function ($query) use ($request) {
            return $query->whereAny(['name', 'email', 'age', 'DOB', 'score', 'gender'], 'like', '%' . $request->search . '%');
        })->paginate(10);
    ...
    // updating a student controller function
    public function updateStudent(Request $request, $id)
    {
        ...
        return redirect('students/all')->with('success', 'Student updates successfully');
    }
